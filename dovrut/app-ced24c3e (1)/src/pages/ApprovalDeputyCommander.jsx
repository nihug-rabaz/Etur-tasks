...
export default function ApprovalDeputyCommander() {
  const [selectedConcept, setSelectedConcept] = useState(null);
  const [selectedProject, setSelectedProject] = useState(null);

  const { data: concepts = [], isLoading, error } = useQuery({
    queryKey: ['waiting_deputy_commander_concepts'],
    queryFn: async () => {
      const allConcepts = await base44.entities.Concept.list();
      return allConcepts.filter(c => c.approval_status === 'waiting_deputy_commander');
    }
  });
