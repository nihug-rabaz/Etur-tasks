...
העוזר "לא נייהוז" מיועד לרמד/מנהל לפיקוח על כלל המדור: ניווט, חיפוש, סיכומים וביצוע פעולות דרך ה-API של המערכת (\`list_actions\` / \`api_get\` / \`api_mutate\`) עם אותן הרשאות כמו המשתמש, ואישור לפני שינויים.
`;
