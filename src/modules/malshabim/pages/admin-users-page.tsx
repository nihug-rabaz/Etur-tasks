...
export function MalshabimAdminUsersPage() {
  ...
  const load = useCallback(async () => {
    const response = await fetch("/api/malshabim/admin/users");
  ...
  return (
    <div className={`${pageShellClass} max-w-4xl`} dir="rtl">
      ...
      <div className={`${panelClass} p-4`}>
        ...
        {message ? (
          <p className="mt-2 text-xs font-semibold text-accent-primary">{message}</p>
        ) : null}
      </div>
