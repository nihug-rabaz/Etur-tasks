...
import { DOMAIN_LABELS, WORK_STATUS_LABELS } from "@/modules/dovrut/lib/approval-flows";
...
export function DovrutProjectDetailsPage({ projectId }: { projectId: string }) {
  ...
  return (
    <div className="mx-auto flex w-full max-w-5xl flex-col gap-5 px-3 py-4 sm:px-0 sm:py-0">
      <div>
        <Link href="/dovrut/projects" className="text-xs font-bold text-accent-primary">
          ← חזרה לפרויקטים
        </Link>
        <h1 className="mt-2 break-words text-xl font-bold text-text-primary">{project.name}</h1>
        {project.campaign_name ? (
          <p className="mt-1 text-xs font-semibold text-accent-primary">קמפיין · {project.campaign_name}</p>
        ) : null}
        {project.description ? (
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          {project.status !== "completed" ? (
            <button
              ...
              className="rounded-xl bg-surface-2 px-3 py-1.5 text-xs font-bold disabled:opacity-40"
            >
              סיים ושלח לארכיון
            </button>
          ) : null}
 ...
 : "bg-surface-2 text-text-secondary "
      ...
      <ul className="space-y-3">
        ...
        {visible.length === 0 ? (
          <p className="py-8 text-center text-sm text-text-muted">אין אייטמים בפרויקט</p>
        ) : null}
      </ul>
    </div>
  );
