        ...
        {filtered.map((project) => (
          <li key={project.id} className="min-w-0">
            ...
            >
              ...
              {project.description ? (
                <p className="mt-1 line-clamp-2 break-words text-xs text-text-secondary">{project.description}</p>
              ) : null}
            </Link>
          </li>
        ))}
        {filtered.length === 0 ? (
          <p className="py-8 text-center text-sm text-text-muted">אין פרויקטים</p>
        ) : null}
      </ul>
    </div>
  );
