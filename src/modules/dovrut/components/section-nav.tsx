...
import { linkAccentClass, panelClass } from "@/modules/dovrut/lib/ui";
...
export function DovrutSectionNav() {
 ...
 : "bg-surface-2 text-text-primary hover:bg-accent-primary/12 hover:text-accent-primary"
 }`}
                  >
                  ...
                  {open ? (
                    <div className={`absolute end-0 top-[calc(100%+0.4rem)] z-30 w-[min(16rem,calc(100vw-1.5rem))] ${panelClass} p-2`}>
                      {rows.length === 0 ? (
                        ...
                        rows.map((row) => (
                          <Link
                            ...
                            href={row.href}
                            onClick={() => setOpenKey(null)}
                            className="block rounded-xl px-3 py-2 text-sm font-semibold text-text-primary hover:bg-accent-primary/10"
                          >
                            {row.name}
                          </Link>
                        ))
                      ...
                      <Link
                        href={item.href}
                        onClick={() => setOpenKey(null)}
                        className={`mt-1 block rounded-xl px-3 py-2 text-xs hover:bg-accent-primary/10 ${linkAccentClass}`}
                      >
                        לכל {item.label} →
                      </Link>
                    </div>
